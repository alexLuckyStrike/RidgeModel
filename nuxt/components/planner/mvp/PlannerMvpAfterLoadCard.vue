<template>
  <div class="space-y-4">
    <p class="text-sm text-slate-600">
      Полоски «до/после» (пара изображений на одну нагрузку).
    </p>
    ...
    <div class="space-y-4">
      ...
      >
        ...
        <div class="mt-3 grid gap-3 sm:grid-cols-2">
          <div class="space-y-2">
            <div class="text-xs font-medium text-slate-700">Покой / До нагрузки</div>
            <label class="block">
              <input
                class="hidden"
                type="file"
                accept="image/*"
                @change="(e) => onSetFileChange(e, set.id, 'first')"
              />
              <div
              ...
              >
                ...
                <span class="text-xs text-slate-500">Первая полоска</span>
              </div>
            </label>
            <div v-if="set.first" class="relative overflow-hidden rounded-xl border border-slate-200">
              <img :src="set.first.url" :alt="set.first.name" class="h-24 w-full object-cover" />
              <button
                type="button"
                class="absolute right-2 top-2 flex h-7 w-7 items-center justify-center rounded-full bg-white/90 text-slate-600 shadow"
                @click.stop="removeSetFile(set.id, 'first')"
              >
          ...
          <div class="space-y-2">
            ...
            <label class="block">
              ...
              >
                <span class="text-sm font-medium text-slate-800">Загрузить фото</span>
                <span class="text-xs text-slate-500">Вторая полоска</span>
              </div>
            </label>
